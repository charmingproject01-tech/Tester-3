...
// =========================================
// KONTROL AUDIO UNDANGAN & YOUTUBE VIDEO
// =========================================

// 1. Muat YouTube IFrame API
...
var ytPlayer;
// Pastikan ID audio kamu adalah 'bg-audio' (atau ganti sesuai ID elemen <audio> kamu)
...
function onPlayerStateChange(event) {
    // Ketika video YouTube diputar (PLAYING)
    if (event.data === YT.PlayerState.PLAYING) {
        if (bgAudio && !bgAudio.paused) {
            bgAudio.pause(); // Matikan lagu undangan
        }
    } 
    // Ketika video di-pause atau selesai (PAUSED / ENDED)
    else if (event.data === YT.PlayerState.PAUSED || event.data === YT.PlayerState.ENDED) {
        if (bgAudio && bgAudio.paused) {
            bgAudio.play().catch(function(err) {
                console.log("Autoplay audio terhalang browser policy:", err);
            }); // Nyalakan lagi lagu undangan
        }
    }
}
